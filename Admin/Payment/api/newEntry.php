...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $acc_type = $_POST["role"];
    // Step 1: Insert data into the "users" table to store the default password
    ...
    if ($stmt = $conn->prepare($sql)) {
        if ($stmt->execute()) {
            ...
            $sql = 'INSERT INTO clients (account_number, last_name, middle_name, first_name, citizenship, civil_status, 
            city_address, provincial_address, 
            mailing_address, account_status, phone_num, taxID_num, spouse_name, birth_date, birth_place, date_employed, 
            appointment_status, position, natureOf_work, balance, amountOf_share) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
            ...
            if ($stmt = $conn->prepare($sql)) { 
                ...
                $param_appointment_status = $_POST["appointment_status"];
                $param_position = $_POST["position"];
                $param_natureOf_work = $_POST["natureOf_work"];
                $param_balance = $_POST["balance"];
                $param_amountOf_share = $_POST["amountOf_share"];


                $stmt->bind_param('iissssssssisssssssssi', $param_account_number, $param_lastName, $param_middleName, $param_firstName, $param_citizenship, $param_civil_status, $param_cityAddress, $param_provincialAddress, $param_mailingAddress, $param_account_status, $param_phone_num, $param_taxID_num, $param_spouseName, $param_birth_date, $param_birth_place, $param_date_employed, $param_appointment_status, $param_position, $param_natureOf_work, $param_balance, $param_amountOf_share);
