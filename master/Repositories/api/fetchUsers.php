...
if ($status === 'all') {
    $query = "SELECT clients.*, users.* FROM clients INNER JOIN users ON clients.user_id = users.user_id";
} else {
    $query = "SELECT clients.*, users.* FROM clients INNER JOIN users ON clients.user_id = users.user_id WHERE users.role = ?";
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>" . $counter . "</td>";
        echo "<td> <a class='text-decoration-none text-primary' href='/coop/master/repositories/edit/edit.php?account_number=" . $row["account_number"] . "'>" . $row["account_number"] . "</td>";
        ...
        echo "<td>";
            ...
            if($row["role"] === 'admin' || $row["role"] === 'master'){
                echo '<a href="/coop/master/repositories/edit/editAdmin.php?account_number=' . $row["account_number"] . '&role=' . $row["role"] . '"><button class="btn btn-success me-1">Edit</button></a>';
            } else if($row["role"] === 'mem') {
                echo '<a href="/coop/master/repositories/edit/edit.php?account_number=' . $row["account_number"] . '"><button class="btn btn-success me-1">Edit</button></a>';
            }
            echo '<a href="/coop/master/repositories/api/delete.php?account_number=' . $row["account_number"] . '"><button class="btn btn-danger">Delete</button></a>';                                        
        echo "</td>";
        echo "</tr>";
